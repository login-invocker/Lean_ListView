...
import com.invocker.invocker_math.Database.MyDBHelper;
...
Activity context;
    ...
    private void addEvent() {
            ...
            btnChiase.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    MyDBHelper mDB = new MyDBHelper(getContext().getApplicationContext());
                    final SQLiteDatabase sqlDB = mDB.getWritableDatabase();
                    UserScope user=new UserScope(edtNameUser.getText().toString(),score);
                    ContentValues ct = new ContentValues();
                    ct.put("name",user.getName());
                    ct.put("scope",user.getScope());
